...
{
    internal class PracticeExam<T> : Exam<T>
        where T : Question
    {
        public PracticeExam(TimeSpan time, int numOfQuestion
        ...
        public override void show()
        {
            ...
            foreach (var item in Question_Answer)
            {
                ...
                if (item.Key is TrueORFalse || item.Key is Choose_One)
                {
                    ...
                    if (int.TryParse(Console.ReadLine(), out answ) && answ > -1 && answ <= item.Key.All_Answer.Count)
                    {
            ...
            Console.WriteLine("****************");
            ///show Model Answer
            showModelAnswers(Question_Answer);
        ...
        public void showModelAnswers(Dictionary<T, AnswerList> valuePairs)
        {
            int countQ = 1;
